...
Adapters that place artifacts into tool-specific locations may use a shorter two-segment form if the target tool has its own namespacing:
...
Given `@shiftbase/golang-expert` with an artifact named `test-writer`:
...
Adapters can use the shorter form because tool-specific directories already provide namespace isolation. A slash command from `@shiftbase/deploy-tools` with a `deploy` command artifact becomes `/shiftbase.deploy`, which reads naturally in tool UIs.
